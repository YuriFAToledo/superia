    ...
    return (
        <div className="flex flex-col items-start gap-4 w-3/4">
                ...
                <Dialog.Root open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                    ...
                    <Dialog.Portal>
                        ...
                        <Dialog.Content className="fixed left-[50%] top-[50%] max-h-[85vh] w-[90vw] max-w-[450px] translate-x-[-50%] translate-y-[-50%] rounded-lg bg-white p-6 shadow-lg">
                            ...
                            <Dialog.Description className="text-sm text-gray-500 mb-4">
                                Preencha as informações para adicionar um novo membro à sua conta.
                            </Dialog.Description>
                                        ...
                                        <div className="col-span-3 flex gap-4">
                                            <label className="flex items-center gap-2">
                                                <input
                                                    ...
                                                    name="role"
                                                    value="user"
                                                    checked={newMemberRole === "user"}
                                                    onChange={() => setNewMemberRole("user")}
                                                />
                                                Usuário
                                            </label>
                                            <label className="flex items-center gap-2">
            ...
            <div className="bg-white border border-border rounded-lg p-4 w-full flex-1 overflow-hidden flex flex-col">
                            ...
                            {loadingMembers ? (
                                ...
                                members.map((member) => (
                                    <TableRow key={member.id}>
                                        <TableCell>
                                            {member.user_metadata?.display_name || "Sem nome"}
                                        </TableCell>
                                                    ...
                                                    <Tooltip>
                                                        <TooltipTrigger asChild>
                                                            <Button
                                                                variant="ghost"
                                                                size="icon"
                                                                onClick={() => handleResendInviteOrResetPassword(member.email || "")}
                                                                disabled={!isAdmin || loading || !member.email}
                                                    ...
                                                    <Tooltip>
                                                        <TooltipTrigger asChild>
                                                            <Button
                                                                ...
                                                                disabled={!isAdmin || loading || member.id === currentUser?.id}
                                                                title="Remover"
                                                            >
                ...
                {!loadingMembers && members.length > 0 && totalPages > 1 && (
