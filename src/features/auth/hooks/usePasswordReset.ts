...
export function usePasswordReset() {
      ...
      setTimeout(() => {
        ...
        if (hash) {
          ...
          if (token) {
            ...
            setLocalError("Link de redefinição de senha inválido ou expirado. Solicite um novo link.")
          }
        } else {
          setLocalError("Nenhum token encontrado na URL. Verifique se o link está correto.")
        }
      }, 500)
    }
  }, [])
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLocalError(error instanceof Error ? error.message : "Erro ao redefinir senha. Tente novamente.")
    }
  }
